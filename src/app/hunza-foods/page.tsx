...
const hunzaProducts = PRODUCTS.filter(p => p.category === "Hunza foods");
      ...
      <section className="container mx-auto px-4 mb-10">
        <div className="grid grid-cols-1 sm:grid-cols-2 md:grid-cols-3 lg:grid-cols-4 gap-8">
          {hunzaProducts.map((product) => (
            <div key={product.id.toString()} className="bg-white rounded-2xl shadow-lg p-6 flex flex-col items-center hover:shadow-emerald-300 transition-all group relative">
              ...
              <h3 className="font-bold text-lg text-emerald-900 mb-2 text-center">{product.title}</h3>
              <span className="text-emerald-600 font-bold text-xl mb-1">PKR {product.newPrice}</span>
              <span className="text-gray-500 line-through text-sm">PKR {product.oldPrice}</span>
              <span className="text-yellow-500 font-semibold text-sm mt-2">{product.reviewStars} ★ ({product.reviews})</span>
