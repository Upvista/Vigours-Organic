...
const desiProducts = PRODUCTS.filter(p => p.category === "Desi foods");
      ...
      <section className="container mx-auto px-4 mb-10">
        <div className="grid grid-cols-1 sm:grid-cols-2 md:grid-cols-3 lg:grid-cols-4 gap-8">
          {desiProducts.map((product) => (
            <div key={product.id.toString()} className="bg-white rounded-2xl shadow-lg p-6 flex flex-col items-center hover:shadow-yellow-300 transition-all group relative">
              ...
              <h3 className="font-bold text-lg text-yellow-900 mb-2 text-center">{product.title}</h3>
              <span className="text-yellow-600 font-bold text-xl mb-1">PKR {product.newPrice}</span>
